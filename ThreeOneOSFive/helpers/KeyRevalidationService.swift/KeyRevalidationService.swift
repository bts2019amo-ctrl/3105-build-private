...
enum KeyRevalidationService {
    ...
    enum ValidationError: Error { case invalidURL, server, response }
    ...
    static func validate(_ key: String) async throws -> KeyRevalidationResult {
        ...
        guard let primaryURL = URL(string: "https://proxysystem.org/api/trpc/iOS.validateKey?batch=1&input=\(encoded)") else { throw ValidationError.invalidURL }
        ...
        if (response as? HTTPURLResponse)?.statusCode == 404 {
            ...
            guard let fallbackURL = URL(string: "https://proxysystem.org/api/trpc/android.validateKey?batch=1&input=\(encoded)") else { throw ValidationError.invalidURL }
        ...
        guard let result = try JSONDecoder().decode([KeyTRPCResponse].self, from: data).first?.result.data.json else { throw ValidationError.response }
