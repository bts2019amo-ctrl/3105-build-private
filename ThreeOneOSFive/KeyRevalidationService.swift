...
enum KeyRevalidationService {
    ...
    static func validate(_ key: String) async throws -> KeyRevalidationResult {
        ...
        guard let primaryURL = URL(string: "https://proxysystem.org/api/trpc/iOS.validateKey?batch=1&input=\(encoded)") else {
        ...
        if (response as? HTTPURLResponse)?.statusCode == 404 {
            ...
            guard let fallbackURL = URL(string: "https://proxysystem.org/api/trpc/android.validateKey?batch=1&input=\(encoded)") else {
